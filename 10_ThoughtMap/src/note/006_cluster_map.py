...
plt.title("ThoughtMap Notes Cluster Map")
plt.xlabel("Axis 1")
plt.ylabel("Axis 2")
plt.tight_layout()

map_path = output_dir / "notes_cluster_map.png"
plt.savefig(map_path, dpi=200)

print(f"noteクラスタマップを保存しました: {map_path}")

report_path = output_dir / "notes_cluster_distance_report.txt"

print("noteクラスタ距離レポート作成中...")
...
print(f"noteクラスタ距離レポートを保存しました: {report_path}")