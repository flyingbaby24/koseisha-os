...
with right:
    ...
    search_clicked = st.button("Search similar works", type="primary", use_container_width=True)
...
if data is not None and not last_error:
    ...
    if not results:
        ...
        best_similarity = float(result_df["similarity"].max()) if not result_df.empty else 0.0
        st.markdown(
            f"""
            <div class="tm-overview-grid">
                <div class="tm-stat-card">
                ...
                <div class="tm-stat-card">
                    <div class="tm-stat-label">Best similarity</div>
                    <div class="tm-stat-value">{best_similarity:.3f}</div>
                </div>
                <div class="tm-stat-card">
